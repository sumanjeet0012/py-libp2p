...
def signature_validator(msg: rpc_pb2.Message) -> bool:
    ...
    try:
        ...
        if msg_pubkey:
            logger.debug("Extracted public key from peer ID, type: %s", msg_pubkey.get_type())
    ...
    if msg_pubkey is None:
        if not msg.key or len(msg.key) == 0:
            logger.debug("Reject because no key attached and cannot extract from peer ID for msg: %s", msg)
            return False
        logger.debug("Using msg.key for signature verification, length: %d", len(msg.key))
        try:
